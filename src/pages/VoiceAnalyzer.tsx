...
import { Mic, Square, RotateCcw, Globe, ChevronDown, AudioLines, Brain } from "lucide-react";
...
function getGradeLabel(score: number): { label: string; color: string } {
  if (score >= 80) return { label: "Strong", color: "hsl(var(--primary))" };
  if (score >= 60) return { label: "Good", color: "hsl(72 100% 63%)" };
  return { label: "Developing", color: "hsl(var(--destructive))" };
}
...
export default function VoiceAnalyzerPage() {
  ...
  return (
    <div className="space-y-8">
      ...
      {/* Recording Area */}
      <div className="glass-card p-10 flex flex-col items-center gap-6 min-h-[280px] justify-center">
        <WaveformVisualizer isRecording={state === "recording"} />

        <AnimatePresence mode="wait">
          {state === "idle" && (
            <motion.div key="idle" initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} exit={{ opacity: 0, scale: 0.95 }} className="flex flex-col items-center gap-5">
              <div className="text-center space-y-1">
                <p className="text-foreground/70 text-sm">
                  {isSpanish ? "Habla durante 30–60 segundos para mejores resultados" : "Speak for 30–60 seconds for best results"}
                </p>
                <p className="text-muted-foreground text-xs">
                  {isSpanish ? "Toca el botón para comenzar" : "Tap to start recording"}
                </p>
              </div>
              <button
                onClick={handleRecord}
                className="orb-button group relative w-24 h-24 rounded-full flex items-center justify-center hover:scale-[1.06] transition-transform"
                style={{
                  background: 'linear-gradient(135deg, hsl(var(--primary)), hsl(72 80% 45%))',
                  color: 'hsl(var(--primary-foreground))',
                  boxShadow: '0 0 30px rgba(232,255,71,0.3), 0 0 60px rgba(232,255,71,0.1)',
                }}
                onMouseEnter={(e) => { e.currentTarget.style.boxShadow = '0 0 40px rgba(232,255,71,0.45), 0 0 80px rgba(232,255,71,0.15)'; }}
                onMouseLeave={(e) => { e.currentTarget.style.boxShadow = '0 0 30px rgba(232,255,71,0.3), 0 0 60px rgba(232,255,71,0.1)'; }}
              >
                {/* Concentric idle rings */}
                <span className="absolute inset-[-8px] rounded-full border border-primary/20 animate-pulse" />
                <span className="absolute inset-[-18px] rounded-full border border-primary/10" />
                <span className="absolute inset-[-30px] rounded-full border border-primary/5" />
          ...
          {state === "recording" && (
            <motion.div key="recording" initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} exit={{ opacity: 0, scale: 0.95 }} className="flex flex-col items-center gap-5">
              {/* Large timer */}
              <div className="text-center space-y-1">
                <p className="text-4xl font-mono font-bold text-foreground tracking-wider">{formatTime(recordingTime)}</p>
                <div className="flex items-center justify-center gap-2">
                  <span className="w-2.5 h-2.5 rounded-full animate-pulse bg-destructive" />
                  <p className="text-sm font-medium text-destructive">{isSpanish ? "Grabando..." : "Recording..."}</p>
                </div>
              </div>
              {/* Stop orb with expanding rings */}
              <div className="relative flex items-center justify-center">
                <span className="absolute w-24 h-24 rounded-full bg-destructive/10 animate-[ping_2s_ease-in-out_infinite]" />
                <span className="absolute w-32 h-32 rounded-full bg-destructive/5 animate-[ping_2.5s_ease-in-out_infinite_0.3s]" />
                <span className="absolute w-40 h-40 rounded-full bg-destructive/[0.03] animate-[ping_3s_ease-in-out_infinite_0.6s]" />
                <button
                  onClick={handleStop}
                  className="relative z-10 w-24 h-24 rounded-full flex items-center justify-center text-white hover:scale-105 transition-transform bg-destructive"
          ...
          {state === "analyzing" && (
            <motion.div key="analyzing" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="flex flex-col items-center gap-5 py-8">
              <div className="relative w-16 h-16">
                <div className="absolute inset-0 rounded-full animate-spin border-[3px] border-primary border-t-transparent" />
                <div className="absolute inset-2 rounded-full animate-spin border-[2px] border-destructive/40 border-b-transparent" style={{ animationDirection: 'reverse', animationDuration: '1.5s' }} />
              </div>
          ...
          <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.5 }} className="space-y-6">
            ...
            <div className="glass-card p-8 flex flex-col items-center gap-3">
              ...
              <p
                ...
                style={{ color: getGradeLabel(analysis.score).color }}
              >
                {getGradeLabel(analysis.score).label}
              </p>
              <p className="text-xs text-muted-foreground">Overall speaking quality</p>
            ...
            <div className="glass-card p-6 space-y-5">
              <h3 className="text-sm font-medium text-muted-foreground">Detailed Breakdown</h3>
              ...
              <ul className="space-y-3">
                {analysis.feedback.map((item, i) => (
                  <li key={i} className="flex gap-3 text-sm">
                    <span className="mt-0.5 w-5 h-5 rounded-full flex items-center justify-center text-xs font-bold shrink-0 bg-primary/10 text-primary border border-primary/20">{i + 1}</span>
            ...
            <Collapsible open={transcriptOpen} onOpenChange={setTranscriptOpen}>
              <div className="glass-card overflow-hidden">
                <CollapsibleTrigger className="w-full p-5 flex items-center justify-between text-left hover:bg-primary/[0.02] transition-colors">
                  ...
                  <ChevronDown className={cn("w-4 h-4 text-muted-foreground transition-transform duration-200", transcriptOpen && "rotate-180")} />
                </CollapsibleTrigger>
                <CollapsibleContent>
                ...
                <div className="glass-card overflow-hidden">
                  <CollapsibleTrigger className="w-full p-5 flex items-center justify-between text-left hover:bg-primary/[0.02] transition-colors">
                    <div className="flex items-center gap-3">
                      ...
                      <h3 className="text-sm font-medium text-muted-foreground">Acoustic Signature</h3>
                    </div>
                    <ChevronDown className={cn("w-4 h-4 text-muted-foreground transition-transform duration-200", acousticOpen && "rotate-180")} />
                  </CollapsibleTrigger>
                  <CollapsibleContent>
